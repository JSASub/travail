...
// Debug et correction ciblée pour DP et date
function debugDPDateElements() {
  console.log('🔍 === DEBUG DP ET DATE ===');
  
  // Vérifier tous les sélecteurs possibles pour DP
  const dpSelectors = [
  ...
  console.log('📝 Recherche éléments DP:');
  ...
  console.log('📅 Recherche éléments DATE:');
...
// Fonction spécialisée pour la sauvegarde DP/Date
function saveDPDateSpecific() {
  try {
    console.log('💾 Sauvegarde spécifique DP/Date...');
    
    // Diagnostic en temps réel
    debugDPDateElements();
    
    // Chercher l'élément DP par tous les moyens possibles
    let dpElement = null;
    ...
    for (const selector of dpSelectors) {
      ...
      if (element && element.value && element.value.trim()) {
        dpElement = element;
        console.log(`✅ DP trouvé via ${selector}: "${element.value}"`);
        break;
      }
    }
    ...
    for (const selector of dateSelectors) {
      ...
      if (element && element.value && element.value.trim()) {
        dateElement = element;
        console.log(`✅ Date trouvée via ${selector}: "${element.value}"`);
        break;
      }
    }
    
    // Sauvegarder si trouvés
    const dpDateData = {};
    ...
    if (Object.keys(dpDateData).length > 0) {
      ...
      console.log('💾 DP/Date spécifique sauvegardé:', dpDateData);
      return true;
    } else {
      console.log('⚠️ Aucun DP/Date à sauvegarder');
      return false;
    }
    
  } catch (error) {
    console.error('❌ Erreur sauvegarde DP/Date spécifique:', error);
    return false;
  }
}

// Fonction spécialisée pour la restauration DP/Date avec retry agressif
async function restoreDPDateSpecific(maxRetries = 10) {
  console.log('📥 Restauration spécifique DP/Date...');
  
  const storedData = sessionStorage.getItem('jsas_dp_date_specific');
  if (!storedData) {
    console.log('ℹ️ Aucune sauvegarde DP/Date spécifique');
    return false;
  }
  
  try {
    const dpDateData = JSON.parse(storedData);
    console.log('📦 Données DP/Date trouvées:', dpDateData);
    
    // Vérifier l'âge (max 2 heures)
    const age = Date.now() - dpDateData.timestamp;
    if (age > 2 * 60 * 60 * 1000) {
      console.log('🗑️ Données DP/Date trop anciennes');
    ...
    while (retryCount < maxRetries && !restored) {
      retryCount++;
      console.log(`🔄 Tentative restauration DP/Date ${retryCount}/${maxRetries}`);
        ...
        for (const selector of dpSelectors) {
          ...
          if (element) {
            ...
            if (!element.value.trim() || element.value === 'Nom du DP') {
              ...
              element.dispatchEvent(new Event('change', { bubbles: true }));
              console.log(`  ✅ DP restauré via ${selector}: "${dpDateData.dp.value}"`);
              dpRestored = true;
              break;
            } else {
              console.log(`  ⚠️ DP déjà rempli via ${selector}: "${element.value}"`);
              dpRestored = true; // Considérer comme restauré si déjà rempli
              break;
            }
          }
        }
        ...
        for (const selector of dateSelectors) {
          ...
          if (element) {
            if (!element.value.trim()) {
              ...
              element.dispatchEvent(new Event('input', { bubbles: true }));
              element.dispatchEvent(new Event('change', { bubbles: true }));
              console.log(`  ✅ Date restaurée via ${selector}: "${dpDateData.date.value}"`);
              dateRestored = true;
              break;
            } else {
              console.log(`  ⚠️ Date déjà remplie via ${selector}: "${element.value}"`);
              dateRestored = true; // Considérer comme restauré si déjà rempli
              break;
            }
          }
        }
      ...
      // Vérifier si au moins un élément a été restauré
      if (dpRestored || dateRestored) {
        restored = true;
        console.log(`✅ Restauration DP/Date réussie (tentative ${retryCount})`);
        ...
        if (typeof showNotification === 'function') {
          ...
          if (items.length > 0) {
            showNotification(`📥 Restauré: ${items.join(', ')}`, "success");
          }
        }
      ...
      if (retryCount < maxRetries) {
        const delay = 500 * retryCount; // Délai progressif
    ...
    if (!restored) {
      console.log('❌ Impossible de restaurer DP/Date après toutes les tentatives');
      // Diagnostic final
      debugDPDateElements();
    }
    ...
    console.error('❌ Erreur restauration DP/Date spécifique:', error);
    return false;
  }
}

// Configuration des listeners spécialisés DP/Date
function setupDPDateSpecificListeners() {
  console.log('🎛️ Configuration listeners spécialisés DP/Date...');
  ...
  setTimeout(() => {
    ...
    allSelectors.forEach(selector => {
      ...
      if (element && !element.hasAttribute('data-dp-date-specific')) {
        ...
        listenersAttached++;
        console.log(`  ✅ Listener attaché: ${selector}`);
      }
    });
    
    console.log(`✅ ${listenersAttached} listeners DP/Date spécialisés attachés`);
...
function testDPDatePersistence() {
  console.log('🧪 === TEST DP/DATE PERSISTENCE ===');
  ...
  // 3. Voir ce qui est sauvegardé
  const stored = sessionStorage.getItem('jsas_dp_date_specific');
  if (stored) {
    console.log('📦 Données sauvegardées:', JSON.parse(stored));
  }
...
window.saveCompleteSessionF5 = function() {
  ...
  // Sauvegarde spécialisée DP/Date en plus
...
window.restoreCompleteSessionF5 = async function() {
  console.log('📥 Restauration avec correction DP/Date...');
  
  // D'abord la restauration spécialisée DP/Date
  const dpDateRestored = await restoreDPDateSpecific();
  
  // Puis la restauration normale (avec délai pour éviter les conflits)
  setTimeout(async () => {
...
document.addEventListener('DOMContentLoaded', () => {
  setTimeout(() => {
    console.log('🔧 Initialisation correctif DP/Date...');
    setupDPDateSpecificListeners();
    
    // Restauration immédiate spécialisée
    setTimeout(() => {
