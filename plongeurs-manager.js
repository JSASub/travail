...
////
// ===== DIAGNOSTIC DÉTAILLÉ DE LA VALIDATION =====

function debugValidationDetailed(nom, niveau, prerogatives = "") {
  console.log("🔍 === DEBUG VALIDATION DÉTAILLÉ ===");
  console.log("Paramètres reçus:");
  console.log("- Nom:", `'${nom}'`, "(type:", typeof nom, ", length:", nom ? nom.length : 'null', ")");
  console.log("- Niveau:", `'${niveau}'`, "(type:", typeof niveau, ", length:", niveau ? niveau.length : 'null', ")");
  console.log("- Prérogatives:", `'${prerogatives}'`, "(type:", typeof prerogatives, ")");
  ...
  console.log("\n--- Validation du NOM ---");
  if (!nom || typeof nom !== 'string') {
    console.log("❌ Nom requis ou pas string");
    errors.push("Le nom est requis");
  } else {
    const nomTrimmed = nom.trim();
    console.log("- Nom après trim:", `'${nomTrimmed}'`, "(length:", nomTrimmed.length, ")");
    
    if (nomTrimmed.length < 2) {
      console.log("❌ Nom trop court");
      errors.push("Le nom doit contenir au moins 2 caractères");
    } else if (nomTrimmed.length > 50) {
  ...
  console.log("- Niveaux valides:", niveauxValides);
  
  if (!niveau || typeof niveau !== 'string') {
    console.log("❌ Niveau requis ou pas string");
    errors.push("Le niveau est requis");
  } else {
    const niveauTrimmed = niveau.trim();
    console.log("- Niveau après trim:", `'${niveauTrimmed}'`);
    console.log("- Le niveau est-il dans la liste ?", niveauxValides.includes(niveauTrimmed));
    
    if (!niveauxValides.includes(niveauTrimmed)) {
  ...
  console.log("\n--- Validation des PRÉROGATIVES ---");
  if (prerogatives && typeof prerogatives === 'string' && prerogatives.length > 100) {
    console.log("❌ Prérogatives trop longues");
    errors.push("Les prérogatives ne peuvent pas dépasser 100 caractères");
  } else {
    console.log("✅ Prérogatives valides");
  }
  
  console.log("\n--- Vérification des DOUBLONS ---");
  console.log("- Variable 'plongeurs' existe ?", typeof window.plongeurs !== 'undefined');
  console.log("- Nombre de plongeurs actuels:", window.plongeurs ? window.plongeurs.length : 'Variable inexistante');
  
  if (nom && niveau && errors.length === 0) {
    if (typeof window.plongeurs !== 'undefined' && Array.isArray(window.plongeurs)) {
      const existe = window.plongeurs.some(p => {
        const match = p.nom && p.nom.toLowerCase().trim() === nom.toLowerCase().trim() &&
                     p.niveau === niveau;
        if (match) {
          console.log("⚠️ Doublon trouvé:", p);
        }
        return match;
      });
      
      if (existe) {
        console.log("❌ Ce plongeur existe déjà");
        errors.push("Ce plongeur existe déjà dans la liste");
      } else {
        console.log("✅ Pas de doublon");
      }
    } else {
      console.log("✅ Pas de liste existante, pas de doublon possible");
    }
  } else {
    console.log("⏭️ Vérification doublons sautée (erreurs précédentes)");
  }
...
// Version simplifiée de validatePlongeur pour comparaison
function validatePlongeurSimple(nom, niveau, prerogatives = "") {
  ...
  // Validation basique du niveau
  const niveauxValides = ['E4', 'E3', 'E2', 'GP', 'N4/GP', 'N4', 'N3', 'N2', 'N1', 'Plg.Or', 'Plg.Ar', 'Plg.Br', 'Déb.', 'débutant', 'Déb'];
  ...
  console.log("Résultat validation simple:", result);
  console.log("=============================\n");
  
  return result;
}

// Test automatique des cas problématiques
function testCasesProblematiques() {
  console.log("🧪 === TESTS AUTOMATIQUES ===");
  
  const testCases = [
    { nom: "Test", niveau: "N2", pre: "Nitrox", description: "Cas supposé valide" },
    { nom: "", niveau: "N2", pre: "Nitrox", description: "Nom vide" },
    { nom: "Test", niveau: "INVALIDE", pre: "", description: "Niveau invalide" },
    { nom: "Test", niveau: "N2", pre: "", description: "Sans prérogatives" },
    { nom: "A", niveau: "N2", pre: "", description: "Nom trop court" },
    { nom: "Test", niveau: "", pre: "", description: "Niveau vide" }
  ];
  
  testCases.forEach((testCase, index) => {
    console.log(`\n--- TEST ${index + 1}: ${testCase.description} ---`);
    const result = debugValidationDetailed(testCase.nom, testCase.niveau, testCase.pre);
    console.log(`Résultat: ${result.valid ? '✅ VALIDE' : '❌ INVALIDE'}`);
    if (!result.valid) {
      console.log("Erreurs:", result.errors);
    }
  });
...
// Diagnostic de l'état des variables globales
function diagnosticVariablesGlobales() {
  console.log("🌐 === DIAGNOSTIC VARIABLES GLOBALES ===");
  
  console.log("window.plongeurs:", {
    exists: typeof window.plongeurs !== 'undefined',
    type: typeof window.plongeurs,
    isArray: Array.isArray(window.plongeurs),
    length: window.plongeurs ? window.plongeurs.length : 'N/A',
    sample: window.plongeurs && window.plongeurs.length > 0 ? window.plongeurs[0] : 'Aucun élément'
  });
  
  console.log("window.plongeursOriginaux:", {
    exists: typeof window.plongeursOriginaux !== 'undefined',
    type: typeof window.plongeursOriginaux,
    isArray: Array.isArray(window.plongeursOriginaux),
    length: window.plongeursOriginaux ? window.plongeursOriginaux.length : 'N/A'
  });
  
  console.log("window.validatePlongeur:", {
    exists: typeof window.validatePlongeur !== 'undefined',
    type: typeof window.validatePlongeur
  });
...
window.debugValidationDetailed = debugValidationDetailed;
window.validatePlongeurSimple = validatePlongeurSimple;
window.testCasesProblematiques = testCasesProblematiques;
window.diagnosticVariablesGlobales = diagnosticVariablesGlobales;

console.log("🔧 Fonctions de debug chargées. Utilisez :");
console.log("- debugValidationDetailed('Test', 'N2', 'Nitrox')");
console.log("- validatePlongeurSimple('Test', 'N2', 'Nitrox')");
console.log("- testCasesProblematiques()");
console.log("- diagnosticVariablesGlobales()");
////
