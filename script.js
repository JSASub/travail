...
function exportToPDF() {
  ...
  try {
    ...
    doc.text('PALANQUEES JSAS', margin + 35, 30);
    ...
    doc.text(dpDateClean, margin + colWidth, metaY + 8);
    doc.text(dpLieuClean, margin + (colWidth * 2), metaY + 8);
    doc.text(dpPlongeeClean, margin + (colWidth * 3), metaY + 8);
    ...
    if (Object.keys(statsNiveaux).length > 0) {
      ...
      const barHeight = 8;
      const maxCount = Math.max.apply(Math, niveauEntries.map(function(entry) { return entry[1]; }));
      
      for (let i = 0; i < niveauEntries.length; i++) {
        const entry = niveauEntries[i];
        const niveau = entry[0];
        const count = entry[1];
        const barY = yPosition + (i * 12);
    ...
    if (palanquees.length === 0) {
      ...
      for (let i = 0; i < palanquees.length; i++) {
        ...
        if (pal.length === 0) {
          ...
          doc.text('Aucun plongeur assigne', margin + 10, yPosition + 5);
          yPosition += 8;
        } else {
          doc.setTextColor(...colors.dark);
          doc.setFontSize(10);
    ...
    for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
      ...
      if (pageNum === totalPages) {
        ...
        const footerSections = [
          { title: 'LEGAL', content: ['Document officiel JSAS', 'Conforme FFESSM'] },
          { title: 'SECURITE', content: ['Verification prerogatives', 'Respect ratios'] },
          { title: 'TECHNIQUE', content: ['Version 2.1.3 Pro', 'Genere le ' + new Date().toLocaleDateString('fr-FR')] }
        ];
        
        const sectionWidth = contentWidth / 3;
        for (let i = 0; i < footerSections.length; i++) {
          const section = footerSections[i];
          const sectionX = margin + (i * sectionWidth);
          ...
          for (let j = 0; j < section.content.length; j++) {
            doc.text(section.content[j], sectionX, footerY + 15 + (j * 4));
          }
          doc.setFont(undefined, 'bold');
          doc.setFontSize(8);
        }
