...
function updateFloatingPlongeursList() {
    ...
    // Récupérer les IDs des plongeurs dans les palanquées depuis window.palanquees
    const plongeursInPalanquees = new Set();
    window.palanquees.forEach(palanquee => {
        palanquee.plongeurs.forEach(plongeur => {
            plongeursInPalanquees.add(plongeur.id);
        });
    });
    
    // Filtrer les plongeurs disponibles
    ...
    if (plongeursDisponibles.length === 0) {
            ...
            <div class="plongeur-item" 
                ...
                <div class="plongeur-info">
                    ...
                    <div class="plongeur-details">
                        ...
                        ${plongeur.specialites && plongeur.specialites.length > 0 ? 
                            `<div class="specialites">${plongeur.specialites.join(', ')}</div>` : ''
                        }
                    </div>
                </div>
            </div>
        ...
        if (typeof attachFloatingDragEvents === 'function') {
