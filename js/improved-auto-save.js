// ps/improved-auto-save.js
// Système de sauvegarde automatique amélioré pour JSAS
// Remplace le système basique existant dans main-core.js
...
(function() {
    ...
    function saveApplicationState() {
        try {
            // Vérifier qu'il y a des données significatives à sauvegarder
            ...
            let dataCount = 0;
            if (plongeurs.length > 0) dataCount += plongeurs.length;
            if (palanquees.length > 0) {
                dataCount += palanquees.reduce((total, pal) => total + (pal?.length || 0), 0);
            }
            if (dpSelect?.value) dataCount++;
            if (dpDate?.value) dataCount++;
            if (dpLieu?.value?.trim()) dataCount++;
            ...
            const appState = {
                ...
                metadata: {
                    dp: {
                        selectedId: dpSelect?.value || '',
                        selectedText: dpSelect?.selectedOptions[0]?.text || '',
                        selectedIndex: dpSelect?.selectedIndex || 0
                    },
                    date: dpDate?.value || '',
                ...
                stats: {
                    ...
                    totalEnPalanquees: palanquees?.reduce((total, pal) => total + (pal?.length || 0), 0) || 0,
    ...
    function checkForSavedData() {
        try {
            ...
            if (age > maxAge) {
                console.log('🗑️ Données de sauvegarde expirées, suppression');
                localStorage.removeItem(CONFIG.STORAGE_KEY);
                return null;
            }

            // Vérifier qu'il y a des données significatives
            const totalData = (appState.stats?.totalPlongeurs || 0) + 
                             (appState.stats?.totalEnPalanquees || 0);
    ...
    async function restoreApplicationState(appState) {
        ...
        try {
            ...
            if (appState.data.plongeurs) {
                window.plongeurs = appState.data.plongeurs;
            }
            if (appState.data.palanquees) {
                window.palanquees = appState.data.palanquees;
            }
            if (appState.data.plongeursOriginaux) {
            ...
            // 3. Restaurer le DP sélectionné (CORRECTION PRINCIPALE)
            if (appState.metadata.dp.selectedId) {
                ...
                <div class="restore-body">
                    <div class="restore-info">
                        <div class="restore-stats">
                            📊 <strong>${appState.stats.totalPlongeurs}</strong> plongeurs, 
                            <strong>${appState.stats.nombrePalanquees}</strong> palanquées
                        </div>
                        <div class="restore-dp">
                            👨‍💼 DP: <strong>${appState.metadata.dp.selectedText || 'Non sélectionné'}</strong>
                        </div>
                        <div class="restore-date">
                            📅 ${appState.metadata.date ? new Date(appState.metadata.date).toLocaleDateString('fr-FR') : 'Date non définie'}
                        </div>
    ...
    function showRestoreSuccessMessage(appState) {
        ...
        message.innerHTML = `
            ...
            <span style="font-size: 12px; opacity: 0.8; margin-left: 10px;">
                ${appState.stats.totalPlongeurs} plongeurs, ${appState.stats.nombrePalanquees} palanquées
            </span>
        `;
    ...
    function setupChangeListeners() {
        ...
        console.log('👁️ Surveillance des changements activée');
    }
    ...
    function setupBeforeUnloadSave() {
        ...
        // Sauvegarder également lors de la perte de focus (navigation, onglet caché, etc.)
    ...
    function disableOldSaveSystems() {
        ...
        const oldKeys = [
            ...
            'emergency_save_jsas',
            'jsas_backup_data',
            'jsas_emergency_backup',  // ← Clé de offline-manager.js
            'jsas_last_backup'        // ← Clé de offline-manager.js
        ];
        ...
        if (window.emergencySave) {
            window.emergencySave = () => console.log('⚠️ Ancien système désactivé');
        }
        if (window.checkEmergencyRestore) {
            window.checkEmergencyRestore = () => console.log('⚠️ Ancien système désactivé');
        }
    }
    ...
    if (document.readyState === 'loading') {
        ...
        setTimeout(() => {
            disableOldSaveSystems();
            initAutoSaveSystem();
        }, 500); // Plus rapide pour prendre la main avant l'ancien système
    }
